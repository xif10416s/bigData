...
package com.fxi.test.spark.graphx
...
object ConnectedComponentsExample {
  def main(args: Array[String]): Unit = {
    ...
    val graph = GraphLoader.edgeListFile(sc, "../bigData/spark/data/graphx/followers.txt")
    // Find the connected components
    val cc = graph.connectedComponents().vertices
    graph.connectedComponents().vertices.foreach(println _)
    // Join the connected components with the usernames
    val users = sc.textFile("../bigData/spark/data/graphx/users.txt").map { line =>
